...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: t('validation_meta_title'),
    description: t('validation_meta_description'),
  };
}

export default async function ValidationDocPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations('docsPages');
  const path = join(process.cwd(), 'docs', 'VALIDATION_PROTOCOL.md');
  ...
  return (
    <main className="mx-auto min-h-[60vh] max-w-3xl px-4 py-10 pb-20">
      ...
      <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
        <h1 className="mb-2 text-2xl font-bold text-slate-900 sm:text-3xl">{t('validation_page_title')}</h1>
        <p className="mb-8 text-sm leading-relaxed text-slate-600">{t('validation_intro')}</p>
